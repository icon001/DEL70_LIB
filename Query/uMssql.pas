    ...
    Function UpdateTB_DEVICECARDNOFromCardGroup(aPromiseGrade,aCardNO:string):string;
{
    ...
    Function DropTB_ZONEDEVICE_ZONENUMIndex : string;
  end;
...
var
  stSql : string;
begin
  stSql := 'select a.AT_ATCODE,a.EM_PASS,c.CO_NAME as COMPANYNAME,a.EM_COTELENCRYPT, ';
  stSql := stSql + ' d.CO_NAME as JIJUMNAME,e.CO_NAME as DEPARTNAME,f.PO_NAME,a.EM_CODE,';
  stSql := stSql + ' a.EM_NAME,a.CO_COMPANYCODE,a.CO_JIJUMCODE,a.CO_DEPARTCODE,a.PO_POSICODE, ';
  stSql := stSql + ' a.EM_COPHONE,a.EM_JOINDATE,a.EM_RETIREDATE,a.ZI_ZIPCODE,';
...
begin
  ...
  if (Not IsMaster) and (aFoodArea = '') then
  begin
...
begin
  ...
  if (Not IsMaster) and (aFoodArea = '') then
  begin
    stSql := stSql + ' Inner Join TB_ADMINFOOD sc ';
    stSql := stSql + ' ON (sa.GROUP_CODE = sc.GROUP_CODE AND sa.AC_NODENO=sc.AC_NODENO ';
    stSql := stSql + ' AND sa.AC_ECUID = sc.AC_ECUID AND sa.FO_DOORNO = sc.FO_DOORNO ';
    stSql := stSql + ' AND sc.AD_USERID = ''' + Master_ID + ''') ';
  end;
  ...
  if aFoodCode <> '' then stSql := stSql + ' AND sa.FO_FOODCODE = ''' + aFoodCode + ''' ' ;
  stSql := stSql + ' ) aa   ';
...
begin
  ...
  stSql := stSql + ' from (select * from TB_ARMAREA where AR_USE = ''Y'' ) d ' ;
  if Not IsMaster then
  begin
    if BuildingGrade = 4 then
    begin
      ...
      stSql := stSql + ' AND c.AC_NODENO = d.AC_NODENO ';
      stSql := stSql + ' AND c.AC_ECUID = d.AC_ECUID  ';
      stSql := stSql + ' AND c.AR_AREANO = d.AR_AREANO ) ';
    end;
  end;
...
begin
  ...
  stSql := stSql + ' where a.GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND a.AC_DATE BETWEEN ''' + aFromDate + ''' AND ''' + aToDate + ''' ';
  if aEmCode <> '' then stSql := stSql + ' AND c.EM_CODE = ''' + aEmCode + ''' ';
...
var
  stSql : string;
begin
  stSql := ' select a.AR_NAME as AL_ZONENAME,b.LO_NAME as buildingname,c.LO_NAME as floorname,d.LO_NAME as areaname ';
  ...
  stSql := stSql + ' where a.GROUP_CODE = '''+GROUPCODE + ''' ';
  stSql := stSql + ' and a.AR_AREANO = ''00'' ';
  if (aBuildingCode <> '') and (aBuildingCode <> '000') then
    stSql := stSql + ' and a.LO_DONGCODE = ''' + aBuildingCode + ''' ';
  if (aFloorCode <> '') and (aFloorCode <> '000') then
    stSql := stSql + ' and a.LO_FLOORCODE = ''' + aFloorCode + ''' ';
  ...
  stSql := stSql + ' and a.AC_NODENO = e.AC_NODENO ';
  stSql := stSql + ' and a.AC_ECUID = e.AC_ECUID ';
  stSql := stSql + ' AND a.AR_AREANO = e.AL_ZONECODE ';
...
begin
  ...
  if (Not IsMaster) and (aFoodArea = '') then
  begin
    stSql := stSql + ' Inner Join TB_ADMINFOOD sd ';
    stSql := stSql + ' ON (sa.GROUP_CODE = sd.GROUP_CODE AND sa.AC_NODENO=sd.AC_NODENO ';
    stSql := stSql + ' AND sa.AC_ECUID = sd.AC_ECUID AND sa.FO_DOORNO = sd.FO_DOORNO ';
    stSql := stSql + ' AND sd.AD_USERID = ''' + Master_ID + ''') ';
  end;
  stSql := stSql + ' Where sa.FO_DATE BETWEEN ''' + aStarDate + ''' ';
  stSql := stSql + ' AND ''' + aEndDate + ''' ';
  if aFoodPermit <> '' then stSql := stSql + ' AND sa.FO_PERMIT = ''' + aFoodPermit + '''  ';
...
begin
  ...
  stSql := stSql + ' SUM(aa.FO_FOODAMT) as FO_FOODAMT ';
  stSql := stSql + ' From  ';
  stSql := stSql + ' ( select sa.GROUP_CODE,sa.FO_DATE,sc.CO_COMPANYCODE,sc.EM_CODE,sc.CO_JIJUMCODE,sc.CO_DEPARTCODE,sc.EM_NAME,  ';
  stSql := stSql + ' CASE  ';
  stSql := stSql + ' WHEN sa.FO_FOODCODE = ''001'' THEN 1  ';
  stSql := stSql + ' ELSE 0 END as FO_BREAK,  ';
  stSql := stSql + ' CASE  ';
...
begin
  stSql := 'Select c.CO_NAME as COMPANY_NAME,d.CO_NAME as JIJUM_NAME,e.CO_NAME as DEPART_NAME,';
  stSql := stSql + ' a.CO_COMPANYCODE,a.EM_CODE,a.EM_NAME,f.AT_DATE,f.AT_ATCODE,j.AT_CODENAME,f.AT_INTIME,f.AT_LEAVETIME,f.AT_BACKTIME,f.AT_OUTTIME,I.PO_NAME,';
  stSql := stSql + ' f.AT_INCODE,g.AT_INNAME as JIKAK,f.AT_OUTCODE,h.AT_OUTNAME as JOTAE,f.AT_UPDATEOPERATOR, ';
  stSql := stSql + ' f.WORKTIME,f.NIGHTWORKTIME, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN f.AT_INRESULT = ''N'' then ''지각'' ';
  stsql := stSql + ' WHEN f.AT_INRESULT = ''Y'' then ''정상'' ';
  stSql := stSql + ' ELSE '''' ';
...
function TMssql.SelectTB_CARDFromArmAreaGradeJoinBase(aAC_NODENO,
...
begin
  ...
  begin
    stSql := stSql + ' AND ( G.DE_USEALARM = ''Y'' ';
    stSql := stSql + ' OR G.DE_ALARM' + aArmAreaNo +' = ''Y'' ) ';
  end else
  begin
...
function TMssql.SelectTB_CARDFromArmAreaGradeNotIn(aAC_NODENO,
  aAC_ECUID,aArmAreaNo: string): string;
var
  stSql : string;
begin
...
//  stSql := stSql + ' AND DE_USEALARM = ''Y'' ';
  ...
  begin
    stSql := stSql + ' AND ( DE_USEALARM  = ''Y'' ';
    stSql := stSql + ' OR  DE_ALARM' + aArmAreaNo + '  = ''Y'') ';
  end else
  begin
...
function TMssql.SelectBuildingTB_ALARMEVENTFromArmAreaDayToDay(aFromDate,
...
begin
  ...
  stSql := stSql + ' a.AL_CHECKCODE,a.AL_CHECKMSG,a.AL_UPDATEOPERATOR,e.AC_DEVICENAME ,';
  stSql := stSql + ' g.LO_NAME as BUILDINGNAME,h.LO_NAME as FLOORNAME,i.LO_NAME as AREANAME,a.al_updatetime,a.AL_CHECKUSER,j.AL_ZONENAME ';
  stSql := stSql + ' From TB_ALARMEVENT a ';
  if Not IsMaster then
  begin
    if BuildingGrade = 4 then
  ...
  stSql := stSql + ' Left Join (select * from TB_ARMAREA Where AR_USE = ''Y'') c ';
  stSql := stSql + ' ON ( a.GROUP_CODE = c.GROUP_CODE ';
  stSql := stSql + ' AND a.AC_NODENO = c.AC_NODENO  ';
  stSql := stSql + ' AND a.AC_ECUID = c.AC_ECUID  ';
  ...
  stSql := stSql + ' AND substring(a.AL_ZONENO,2,1) = j.AL_ZONENUM) ';
  stSql := stSql + ' Where a.GROUP_CODE = ''' + GROUPCODE + ''' ';
  if Not aPcTime then
  begin
    stSql := stSql + ' AND a.AL_DATE BETWEEN ''' + aFromDate
  ...
  begin
    stSql := stSql + ' AND substring(a.AL_INPUTTIME,1,8) BETWEEN ''' + aFromDate
                   + ''' AND ''' + aToDate + ''' ';
  end;
  if Trim(aNodeNo) <> '' then
  begin
...
function TMssql.SelectTB_ARMAREAGRADEJoinBase(aBuildingCode, aFloorCode,
...
{  stSql := stSql + ' INNER JOIN TB_ACCESSDEVICE b ';
  ...
  stSql := stSql + ' AND a.AC_ECUID = c.AC_ECUID  ';
//  stSql := stSql + ' AND c.DE_USEALARM = ''Y'' ';
  stSql := stSql + ' AND ( (a.AR_AREANO = ''00'' AND c.DE_USEALARM = ''Y'') OR ';
  stSql := stSql + '       (a.AR_AREANO = ''00'' AND c.DE_ALARM0 = ''Y'') OR  ';
...
function TMssql.selectTB_ARMAREAJoinAdmin(aBuildingCode, aFloorCode,
...
function TMssql.selectTB_ARMAREAJoinPromiseCode(
  aPromisecode: string): string;
var
  stSql :string;
begin
...
function TMssql.SelectTB_ALARMEVENT_ArmAreaFromDayToDay(aFromDate, aToDate,
  aNodeNo, aEcuId, aArmAreaNo, aAlarmType: string;
...
(*  stSql := 'Select a.AL_DATE,a.AL_TIME,a.AC_NODENO,a.AC_ECUID,c.AR_NAME,a.AL_OPERATOR,a.AL_ALARMDEVICETYPECODE, ';
  stSql := stSql + ' a.AL_ALARMMODECODE,a.AL_ZONENO,a.AL_ALARMSTATUSCODE,a.AL_STATUSCODE2,d.AL_ALARMNAME,a.AL_SUBADDR,';
  ...
  begin
    stSql := stSql + ' AND a.AC_NODENO = ' + inttostr(strtoint(aNodeNo)) ;
    stSql := stSql + ' AND a.AC_ECUID = ''' + aEcuId + ''' ';
    stSql := stSql + ' AND a.AL_ZONECODE = ''' + FillZeroStrNum(aArmAreaNo,2) + ''' ';
  end;
  if Trim(aAlarmType) <> '' then
  begin
    stSql := stSql + ' AND a.AL_ALARMSTATUSCODE = ''' + aAlarmType + ''' ' ;
...
//  else stSql := stsql + ' Order by a.AL_DATE DESC ';
  ...
  stSql := stSql + ' AND substring(a.AL_ZONENO,2,1) = g.AL_ZONENUM) ';
  stSql := stSql + ' Where a.GROUP_CODE = ''' + GROUPCODE + ''' ';
  if Not aPcTime then
  begin
    stSql := stSql + ' AND a.AL_DATE BETWEEN ''' + aFromDate
...
function TMssql.AlterTB_ARMAREATelNo_Add: string;
var
  stSql : string;
begin
...
function TMssql.AlterTB_ARMAREAmemo_Add: string;
var
  stSql : string;
begin
...
function TMssql.UpdateTB_ARMAREA_FromAlarmDeviceMemo: string;
var
  stSql : string;
begin
    stSql := ' Update A ';
...
function TMssql.AlterTB_DEVICECARDNO_HISAlarm0_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM0 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm1_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM1 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm2_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM2 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISAlarm8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_ALARM8 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_HISDoor8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_HIS ADD DE_DOOR8 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm0_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM0 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm1_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM1 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm2_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM2 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEAlarm8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_ALARM8 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNO_PROMISEDoor8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNO_PROMISE ADD DE_DOOR8 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm0_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM0 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm1_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM1 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm2_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM2 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPAlarm8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_ALARM8 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor3_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR3 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor4_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR4 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor5_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR5 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor6_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR6 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor7_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR7 varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_DEVICECARDNOGROUPDoor8_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_DEVICECARDNOGROUP ADD DE_DOOR8 varchar(1) ';
  result := stSql;
end;
...
function TMssql.AlterTB_EMPLOYEE_MASTER_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_EMPLOYEE ADD EM_MASTER char(1)';
  result := stSql;
end;

function TMssql.CreateTB_SONGHOFDCONFIG: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_SEMESTER: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_FOODDayCount: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_FOODSemesterCount: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_FOODWeekCount: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_FOODCodeCount: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_EMPLOYEECHANGE: string;
var
  stSql : string;
begin
...
begin
  ...
  stSql := stSql + ' FD_CHANGE char(1) NULL,';
...
function TMssql.AlterTB_ATWORKTYPE_ATSTARTBUTTON_Change: string;
var
  stSql : string;
begin
  stSql := 'alter table TB_ATWORKTYPE alter column  AW_ATSTARTBUTTON varchar(10) ';
  result := stSql;
end;

function TMssql.AlterTB_ATWORKTYPE_ATOFFBUTTON_Change: string;
var
  stSql : string;
begin
  stSql := 'alter table TB_ATWORKTYPE alter column  AW_ATOFFBUTTON varchar(10) ';
  result := stSql;
end;

function TMssql.AlterTB_ATWORKTYPE_WORKOUTBUTTON_Change: string;
var
  stSql : string;
begin
  stSql := 'alter table TB_ATWORKTYPE alter column  AW_WORKOUTBUTTON varchar(10) ';
  result := stSql;
end;

function TMssql.AlterTB_ATWORKTYPE_WORKINBUTTON_Change: string;
var
  stSql : string;
begin
  stSql := 'alter table TB_ATWORKTYPE alter column  AW_WORKINBUTTON varchar(10) ';
  result := stSql;
end;

function TMssql.AlterTB_EMPLOYEE_CAGROUP_Add: string;
var
  stSql : string;
begin
...
function TMssql.AlterTB_ACCESSDEVICE_MEMLOAD_ADD: string;
var
  stSql : string;
begin
...
function TMssql.AlterTB_DOOR_MEMLOAD_ADD: string;
var
  stSql : string;
begin
...
function TMssql.AlterTB_ARMAREA_MEMLOAD_ADD: string;
var
  stSql : string;
begin
...
function TMssql.AlterTB_EMPLOYEE_EXPIREUSE_ADD: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_EMPLOYEEEXPIRECHANGE: string;
var
  stSql : string;
begin
...
function TMssql.CreateTB_EMPLOYEEEXPIRE_Trigger: Boolean;
var
  stSql : string;
begin
(*
  ...
  stSql := ' 		declare @groupcode varchar(10) ';
  stSql := '    ';
  stSql := ' 		if UPDATE(EM_RETIREDATE)  ';
  stSql := ' 		Begin ';
  stSql := ' 			select @groupcode = GROUP_CODE,@co_companycode = co_companycode, @em_code = em_code,@em_oldretiredate = em_retiredate from deleted ';
  ...
  stSql := ' 	End ';
  *)
  stSql := 'Create Trigger trg_TB_EMPLOYEEEXPIRECHANGE_Update ON TB_EMPLOYEE For Update AS BEGIN  declare @co_companycode varchar(50) declare @em_code varchar(50) declare @em_oldretiredate varchar(8) ';
  stSql := stSql + ' declare @em_newretiredate varchar(8) declare @groupcode varchar(10) if UPDATE(EM_RETIREDATE) Begin select @groupcode = GROUP_CODE,@co_companycode = co_companycode, @em_code = em_code,@em_oldretiredate = em_retiredate from deleted ';
  stSql := stSql + ' select @em_newretiredate = em_retiredate from inserted Insert into TB_EMPLOYEEEXPIRECHANGE(CO_COMPANYCODE,EM_CODE,EM_OLDEXPIRE,EM_NEWEXPIRE) values (@co_companycode,@em_code,@em_oldretiredate,@em_newretiredate) ';
  stSql := stSql + ' update TB_EMPLOYEE set EM_EXPIRUSE = ''N'' where GROUP_CODE = @groupcode and CO_COMPANYCODE = @co_companycode and em_code = @em_code End END ';
  
  result := DataModule1.ProcessExecSQL(stSql);

end;

function TMssql.AlterTB_EMPLOYEE_CardChange_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_EMPLOYEE ADD EM_CARDCHANGE varchar(1) ';
  result := stSql;
end;

function TMssql.AlterTB_EMPLOYEE_COTELENCRYPT_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_EMPLOYEE ADD EM_COTELENCRYPT char(1) NULL ';
  result := stSql;
end;

function TMssql.SelectTB_EMPLOYEEJoinATD2DState(aFromDate,
  aToDate: string): string;
var
  stSql : string;
begin
  stSql := 'Select c.CO_NAME as COMPANY_NAME,d.CO_NAME as JIJUM_NAME,e.CO_NAME as DEPART_NAME,';
  stSql := stSql + ' a.EM_NAME,a.EM_CODE,I.PO_NAME, ';
  stSql := stSql + ' b.cnt,b.intime,b.outtime,b.JIKAK,b.HOLIDAYCNT,b.HOLIDAYINTIME,b.HOLIDAYOUTTIME ';
  stSql := stSql + ' from TB_EMPLOYEE a  ';
  stSql := stSql + ' Inner Join (';
  stSql := stSql + ' select aaa.CO_COMPANYCODE,aaa.EM_CODE,sum(aaa.cnt) as cnt,sum(aaa.intime) as intime,';
  stSql := stsql + ' sum(aaa.outtime) as outtime,sum(aaa.JIKAK) as JIKAK,';
  stSql := stsql + ' sum(aaa.HOLIDAYCNT) as HOLIDAYCNT,sum(aaa.HOLIDAYINTIME) as HOLIDAYINTIME,sum(aaa.HOLIDAYOUTTIME) as HOLIDAYOUTTIME ';
  stSql := stSql + ' from ( select aa.CO_COMPANYCODE,aa.EM_CODE,';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then 0 ';
  stSql := stSql + ' ELSE 1 ';
  stSql := stSql + ' END as cnt, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then 0 ';
  stSql := stSql + ' ELSE substring(aa.at_intime,9,2)*60 + substring(aa.at_intime,11,2) ';
  stSql := stSql + ' END as intime, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then 0 ';
  stSql := stSql + ' ELSE substring(aa.at_outtime,9,2)*60 + substring(aa.at_outtime,11,2) ';
  stSql := stSql + ' END as outtime, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN cc.at_intype = ''2'' then 1 ';
  stSql := stSql + ' ELSE 0 ';
  stSql := stSql + ' END as JIKAK, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then 1 ';
  stSql := stSql + ' ELSE 0 ';
  stSql := stSql + ' END as HOLIDAYCNT, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then substring(aa.at_intime,9,2)*60 + substring(aa.at_intime,11,2) ';
  stSql := stSql + ' ELSE 0 ';
  stSql := stSql + ' END as HOLIDAYINTIME, ';
  stSql := stSql + ' CASE ';
  stsql := stSql + ' WHEN bb.HO_STATE = ''I'' OR aa.AT_WEEKCODE = ''1'' then substring(aa.at_outtime,9,2)*60 + substring(aa.at_outtime,11,2) ';
  stSql := stSql + ' ELSE 0 ';
  stSql := stSql + ' END as HOLIDAYOUTTIME ';
  stSql := stSql + ' from TB_ATEVENT aa ';
  stSql := stSql + ' Left Join TB_HOLIDAY bb ';
  stSql := stSql + ' ON(aa.GROUP_CODE = bb.GROUP_CODE ';
  stSql := stSql + ' AND aa.AT_DATE = bb.HO_DAY ) ';
  stSql := stSql + ' Left Join TB_ATINCODE cc ';
  stSql := stSql + ' ON (aa.GROUP_CODE = cc.GROUP_CODE ';
  stSql := stSql + ' AND aa.AT_INCODE = cc.AT_INCODE ) ';
  stSql := stSql + ' Where aa.AT_DATE BETWEEN ''' + aFromDate + '''  ';
  stSql := stSql + ' AND ''' + aToDate + ''') aaa  ';
  stSql := stSql + ' group by aaa.CO_COMPANYCODE,aaa.EM_CODE ';
  stSql := stSql + ' )b ';
  stSql := stSql + ' ON (a.CO_COMPANYCODE = b.CO_COMPANYCODE ';
  stSql := stSql + ' AND a.EM_CODE = b.EM_CODE ) ';
  stSql := stSql + ' Left Join TB_COMPANY c ';
...
function TMssql.AlterTB_FINGERDEVICE_CHANGE_ADD: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_FINGERDEVICE ADD FD_CHANGE char(1) ';
  result := stSql;

end;

function TMssql.AlterTB_ZONEDEVICE_ZONENUM_Change: string;
var
  stSql : string;
begin
  stSql := 'alter table TB_ZONEDEVICE alter column AL_ZONENUM varchar(2) ';
  result := stSql;
end;

function TMssql.DropTB_ZONEDEVICE_PKIndex: string;
var
  stSql : string;
begin
  result := '';
  stSql := 'select * from information_schema.table_constraints where TABLE_NAME = ''TB_ZONEDEVICE'' ';
  ...
  begin
    ...
    result := 'ALTER TABLE [TB_ZONEDEVICE] DROP CONSTRAINT [' + FindField('CONSTRAINT_NAME').AsString+ ']';
  end;
end;

function TMssql.AlterTB_ZONEDEVICE_PKIndexAdd: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE [TB_ZONEDEVICE] ADD CONSTRAINT [TB_ZONEDEVICE_PKIndex] PRIMARY KEY(GROUP_CODE,AC_NODENO,AC_ECUID,AL_ZONENUM)';
  result := stSql;

end;

function TMssql.DropTB_ZONEDEVICE_ZONENUMIndex: string;
var
  stSql : string;
begin
  result := '';
  stSql := 'exec sp_helpconstraint  [TB_ZONEDEVICE] ';
  stSql := stSql + ' DECLARE @sysname sysname ';
  stSql := stSql + ' DECLARE @sql varchar(1000) ';

  stSql := stSql + ' IF EXISTS (SELECT name FROM sysobjects ';
  stSql := stSql + '          WHERE type =''D'' AND name like ''%TB_ZONEDE%'')  ';
  stSql := stSql + '  BEGIN    ';
  stSql := stSql + '     SET @sysname = (SELECT name FROM sysobjects  ';
  stSql := stSql + '                 WHERE type =''D'' AND name like ''%TB_ZONEDE%'' and xtype = ''D'') ';
  stSql := stSql + '     set @sql = ''ALTER TABLE [TB_ZONEDEVICE] DROP CONSTRAINT '' + @sysname ';
  stSql := stSql + '      exec (@sql) ';
  stSql := stSql + '  END  ';
  DataModule1.ProcessExecSQL(stSql);
end;

function TMssql.AlterTB_ZONEDEVICE_ZONEEXT_ADD: string;
var
  stSql : string;
begin
